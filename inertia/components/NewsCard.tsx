    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-300">
      <div className="flex items-center p-2 gap-4">
        ...
        <div className="flex-grow min-w-0">
          ...
          <span className={`px-2 py-1 text-xs font-medium rounded-full ${type === 'single' ? 'bg-green-100 text-green-800' :
            type === 'album' ? 'bg-purple-100 text-purple-800' :
              type === 'event' ? 'bg-orange-100 text-orange-800' :
                isUpcoming ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
            }`}>
            {type}
          </span>
          {isUpcoming ? <p className="text-xs text-gray-500 mt-1">
            {date}
          </p> : null}
        </div>
