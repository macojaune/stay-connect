...
const TimelineSection: React.FC<TimelineSectionProps> = ({
  ...
  const initialLimit = useMemo(() => {
    ...
    return 2
  }, [isCurrentWeek, isPreviousWeek, section.isUpcoming, section.title])

  const newsToShow = useMemo(() => {
    if (initialLimit === Number.POSITIVE_INFINITY || showMore) {
      return section.news
    }
    return section.news.slice(0, initialLimit)
  }, [initialLimit, section.news, showMore])

  const remainingCount = useMemo(() => {
    if (initialLimit === Number.POSITIVE_INFINITY) {
      return 0
    }
    return Math.max(0, section.news.length - initialLimit)
  }, [initialLimit, section.news.length])

  const canToggleShowMore = initialLimit !== Number.POSITIVE_INFINITY && remainingCount > 0
  ...
  const showMoreButtonText = useMemo(() => {
    ...
    return showMore
      ? 'Voir moins'
      : `Voir plus${remainingCount > 0 ? ` (${remainingCount} autres)` : ''}`
  }, [canToggleShowMore, remainingCount, section.title, showMore])
  ...
  // Fallback rendering for sections displayed as simple news card lists
  return (
    <div key={sectionIndex} className={`relative mb-4 md:mb-8 transition-opacity duration-300`}>
      {/* Timeline dot */}
      <div
