...
import type { SpotifySearchResult } from "@/types/index.js";

export default class SearchSpotifyArtist extends BaseCommand {
  static commandName = "spotify:search-artist";
  static description =
    "Search for artists on Spotify and optionally create them in the database";
  ...
  @flags.boolean({
    description: "Interactive mode to select and create artist",
  })
  ...
  async createArtistFromSpotify(
    ...
    const spotifyService = new SpotifyService();
    return await spotifyService.createArtistFromSpotify(
      spotifyResult,
      additionalData,
    );
  }
  ...
  private async handleInteractiveSelection(results: SpotifySearchResult[]) {
    ...
    try {
      ...
      this.logger.info(
        `   Profile Picture: ${artist.profilePicture || "None"}`,
      );
    } catch (error) {
