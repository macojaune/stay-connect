...
export default class SyncSpotifyArtists extends BaseCommand {
  ...
  async run() {
    ...
    try {
      ...
      this.logger.error('Failed to sync Spotify artists:', error)
      this.exitCode = 1
    }
  }
  ...
  private async syncAllArtists() {
    ...
    for (let i = 0; i < artists.length; i += this.batchSize) {
      ...
      this.logger.info(`Processing batch ${Math.floor(i / this.batchSize) + 1}/${Math.ceil(artists.length / this.batchSize)}`)
      ...
      for (const artist of batch) {
        try {
          ...
          this.logger.error(`Failed to sync artist ${artist.name}:`, error)
          errors++
        }
      }
