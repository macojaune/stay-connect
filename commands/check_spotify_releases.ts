...
export default class CheckSpotifyReleases extends BaseCommand {
  ...
  async run() {
    ...
    try {
      ...
      this.logger.info(`Statistics: Processed ${stats.processed} artists, Found ${stats.newReleases} new releases, ${stats.errors} errors`)
      ...
      this.logger.error('Failed to check Spotify releases:', error)
      this.exitCode = 1
    }
  }
}