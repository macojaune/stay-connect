...
export const rules = {
   ...
   */
  socialMediaUrls: {
    facebook: vine.string().regex(
      /^(https?:\/\/)?(www\.)?facebook\.com\/[a-zA-Z0-9(\.)]+$/,
      'Invalid Facebook URL'
    ).optional(),
    twitter: vine.string().regex(
      /^(https?:\/\/)?(www\.)?twitter\.com\/[a-zA-Z0-9_]+$/,
      'Invalid Twitter URL'
    ).optional(),
    instagram: vine.string().regex(
      /^(https?:\/\/)?(www\.)?instagram\.com\/[a-zA-Z0-9_.]+$/,
      'Invalid Instagram URL'
    ).optional(),
    soundcloud: vine.string().regex(
      /^(https?:\/\/)?(www\.)?soundcloud\.com\/[a-zA-Z0-9-]+$/,
      'Invalid SoundCloud URL'
    ).optional(),
    spotify: vine.string().regex(
      /^(https?:\/\/)?(open\.)?spotify\.com\/(artist|user)\/[a-zA-Z0-9-]+$/,
      'Invalid Spotify URL'
    ).optional(),
    youtube: vine.string().regex(
   ...
   */
  trackDuration: vine.string().regex(
    /^([0-5][0-9]):([0-5][0-9])$/,
    'Invalid track duration format (must be MM:SS)'
  ),
